...
{% include quiz.html question="위상공간의 어떤 원소를 기저의 원소들의 교집합으로 표현하는 방법은 유일하다." options="O,X" answer="2" hint="위상의 기저는 선형대수학의 기저와 비슷하지만 다르다고 했었죠!!" %}
{% include quiz.html question="Lower Limit Topology($\mathbb{R}_l$)와 K-Topology($\mathbb{R}_K$)는 비교가능(comparable)하다." options="O,X" answer="2" %}
